...
const PanelView = ({ children, className, id }: PanelProps) => (
  <div id={id} className={cn("transition-all duration-300 panel-transition", className)}>
    {children}
  </div>
);
...
export function ThreePanelLayout({
  ...
  return (
    <div className={cn("w-full relative", className)}>
      ...
      {!isMobile && (
        <div className="w-full px-4 py-6">
          <PanelView
            ...
            className={cn("w-full transition-opacity duration-300", activePanel === "collections" ? "block" : "hidden")}
          >
            {collectionsPanel}
          </PanelView>
          <PanelView
            id="builder-panel"
            className={cn("w-full transition-opacity duration-300", activePanel === "builder" ? "block" : "hidden")}
          >
            {builderPanel}
          </PanelView>
          <PanelView
      ...
      {isMobile && <MobileBottomNav activePanel={activePanel} onPanelChange={handlePanelChange} />}
    </div>
  );
}
